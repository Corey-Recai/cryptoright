...
const contract_address = '0xb94b4b6576BfA9CB8862D3f42742a0a358cd0658';
...
const dApp = {
  ...
  updateUI: function () {
      ...
      { name, description, image }
        ...
        <li>
          ...
          <div class="collapsible-body">
            <h6>Description</h6>
            <p>${description}</p>
            <img src="https://gateway.pinata.cloud/ipfs/${image.replace(
              'ipfs://',
              ''
            )}" style="width: 100%" />
    ...
    this.contract.events.Copyright({ fromBlock: 0 }, (err, event) => {
      ...
      fetchMetadata(reference_uri).then((json) => {
        $('#dapp-copyrights').append(
          renderItem(copyright_id, reference_uri, 'far fa-copyright', json)
        );
      });
    });
    ...
    this.contract.events.OpenSource({ fromBlock: 0 }, (err, event) => {
      ...
      fetchMetadata(reference_uri).then((json) => {
        $('#dapp-opensource').append(
          renderItem(copyright_id, reference_uri, 'fab fa-osi', json)
        );
      });
    });
  },
